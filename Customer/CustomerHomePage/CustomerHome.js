    ...
    axios.get(`http://localhost:8080/products`)
      .then(function (response) {
        for(var i = 0; i < response.data.length; i++) {
          ...
          allProducts += "<div class=\"col-lg-4 col-md-6 col-sm-6\" align=\"center\"><div class=\"product\" onclick=\"addBasket(" + response.data[i].id + "," + '\'' + response.data[i].name + '\'' + "," + response.data[i].price + ")\"><a><img class=\"img-fluid\" src=images\\unsplash_NOpsC3nWTzY.svg></a><div class=\"bottom\"><h4><img class=\"cart\" src=\"images\\Vector.svg\">$" + response.data[i].price + "</h4></div></div></div>"
        }
